...
| 2026-09-24 | **V1.6.11** | ① **方式二的默认形态由「本地源码构建」改为「云端直拉镜像」（不改接口 / 表结构 / 判定口径）**<br>② **变更**：`deploy/docker-deploy.sh` 的 `USE_IMAGE` 默认值 `false` → `true`，于是默认即云端直拉（等同旧的 `--image`，该参数保留但**可省略**）；新增 `--source` 显式回落本地源码构建形态；`--registry <host>/<命名空间>`（或 `LLMBRIDGE_REGISTRY` 环境变量）可换成**任意第三方镜像库**，不再绑死阿里云 ACR<br>③ **效果**：目标机只要有 Docker + Compose v2 与容器库访问权，一条 `docker run --rm --entrypoint cat <registry>/llmbridge-api:1.0.0 /opt/llmbridge/deploy/docker-deploy.sh \| bash -s --` 即拉起 api + PostgreSQL + Redis + Nginx——**不需要下载源码、不需要本地构建、不需要 Node.js**，且**零 GitHub 请求**（脚本随 api 镜像交付、编排文件内嵌于脚本）<br>④ **文档同步（README 走极简）**：README（中英）「方式二」只留**一条 `docker run` 装完**的简明说明（前置条件 → 命令 → 访问 → 常用命令 → 换镜像库 → 详见部署文档），不再展开两种形态；源码构建由「方式四」承接，细节在 `01-部署文档.md` §3.6、`05-安装打包说明.md` §6.1.1（示例命令已去掉 `--image`）；CHANGELOG 补记录<br>⑤ **兼容性**：`--image` 仍被接受，已在运行的实例不受影响 | HD |
| 2026-09-24 | **V1.6.12** | ① **方式二改为 Sub2API 形态（不改接口 / 表结构 / 判定口径）**：`docker-deploy.sh` 默认动作由 `deploy` 翻为 **`prepare`** —— 只在当前目录生成 `./docker-compose.yml` + `./.env`，**全程不碰 Docker**，启停改用标准 `docker compose up -d` / `docker compose logs -f api` 四步（mkdir → 跑脚本 → up → logs）<br>② **免 `--env-file`**：根目录副本把 `env_file` 由 `../.env` 改写为同目录 `.env`，变量插值与容器环境变量两条路径落在同一份文件；`deploy` 子命令保留为一键全量，`status/logs/down/upgrade/purge` 经 resolve step 0 在 prepare 布局自动识别<br>③ **护栏**：非本项目 `docker-compose.yml` 拒覆盖、旧布局在上一层且 cwd 无 `.env` 时拒绝生成（防空库分叉）、仓库根仅告警、`prepare` 拒 `--source` / 忽略 `--dir`；旧布局仍自动识别（兼容）<br>④ 修复内嵌编排端口漂移 `${HTTP_PORT:-80}` → `${HTTP_PORT:-8081}`（与真源对齐）<br>⑤ **文档同步**：README（中英）方式二改四步快速开始块，`01-部署文档.md` 顶部与 §3.6、`05-安装打包说明.md` §6.0 / §6.1.1 全部改为 prepare → compose 两步口径<br>⑥ **取脚本改为 Sub2API 同款 `curl -sSL <raw...>/deploy/docker-deploy.sh \| bash`**：GitHub 不可达（国内）时保留 `docker run --entrypoint cat` 镜像管道为等价退路（全程零 GitHub）；脚本 `self_cmd` / `--help` 提示与「不需要 GitHub」表述同步修正 | HD |
