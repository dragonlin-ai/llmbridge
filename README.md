...
![llmbridge 架构](.github/images/architecture.zh.svg)

*图：llmbridge 请求处理与路由架构 —— /v1 对外网关 → 路由引擎 → 适配层 → 模型池 → 数据层；/admin 控制台管理配置。*
...
| 部署 | systemd（脚本安装）· Docker Compose（云端直拉）· Apple container（macOS）· 源码 |
