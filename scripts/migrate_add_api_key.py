"""幂等迁移：新建 `api_key` 表（第三方调用密钥，V2「独立 api_key 表」落地）。

表结构由 ORM 定义（app/db/tables/__init__.py: ApiKey），本脚本只负责
在既有库上把缺的表补出来 —— `Base.metadata.create_all` 本身幂等（缺则建、有则跳过），
这里包一层是为了统一给 dry-run 输出与行数校验，与其余迁移脚本口径一致。

用法：
    python scripts/migrate_add_api_key.py --dry-run
...
from app.db.tables import ApiKey, Base  # noqa: E402

TABLE = "api_key"
EXPECTED_COLUMNS = ["id", "name", "key_hash", "key_prefix", "key_encrypted",
                    "enabled", "last_used_at", "expires_at", "created_at", "updated_at"]
